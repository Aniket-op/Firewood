...
app.get('/', (req, res) => {
    ...
    try {
        ...
        // If guest, clear cookie and show home-no-user
        if (user.isGuest) {
            res.clearCookie('token');
...
// Only logged in users (not guests) can create new cabins
app.get('/new-cabin', loginRequired, (req, res) => {
    const cabinAddress = uuidV4();
    res.redirect(`/cabin/${cabinAddress}`);
});

// Join cabin route - allows guests
app.get('/join', (req, res) => {
    // Clear any existing guest tokens
    res.clearCookie('token');
    res.render('join-cabin');
});

// Handle guest joining with cabin code
app.post('/api/join-guest', (req, res) => {
    const { cabinCode, guestName } = req.body;

    if (!cabinCode || !guestName) {
        return res.json({ status: 'error', error: 'Missing cabin code or name.' });
    }

    if (guestName.length < 1 || guestName.length > 30) {
        return res.json({ status: 'error', error: 'Name must be between 1 and 30 characters.' });
    }

    // Sanitize guest name
    const sanitizedName = guestName.trim();

    // Create a guest token that expires when they leave
    const guestId = uuidV4();
    const guestToken = jsonwebtoken.sign({
        id: guestId,
        username: `${sanitizedName} (Guest)`,
        isGuest: true
    }, process.env.JWT_SECRET, { expiresIn: '6h' });

    // Set cookie that expires with session
    res.cookie('token', guestToken, {
        maxAge: 1000 * 60 * 60 * 6, // 6 hours
        httpOnly: true
    });
    return res.json({ status: 'ok', cabinCode: cabinCode });
});

// Cabin route - works for both logged in users and guests
app.get('/cabin/:cabin?', (req, res) => {
    ...
    try {
        ...
        // No valid token, redirect to join page with cabin code
        res.clearCookie('token');
        return res.redirect(`/join?cabin=${req.params.cabin}`);
    }
...
app.get('/register', (req, res) => {
    ...
    if (token) {
        try {
            var user = jsonwebtoken.verify(token, process.env.JWT_SECRET);
            console.log("Current: ", user);
            // Guests can access register page
            if (user.isGuest) {
                res.clearCookie('token');
...
app.post('/api/forgot-password', async (req, res) => {
    ...
    let mail = await transporter.sendMail({
        from: '"Firewood [no-reply]" <app@firewood.vsahni.me>',
        to: user['email'],
...
app.get('/api/state', (req, res) => {
    ...
    if (token) {
        try {
            var user = jsonwebtoken.verify(token, process.env.JWT_SECRET);
            state = user.isGuest ? "guest" : "logged-in";
            ...
            state = "error";
            res.clearCookie('token');
            return res.send(JSON.stringify({ state: state, username: null, isGuest: false }));
        }
    }
    else {
        state = null;
